...
from myapp.application.adapter.api.http.serializer.sucessfullt_voted_result_serializer import (
    SuccessfullyVotedResultSerializer
)
from myapp.application.adapter.api.http.serializer.vote_for_article_command_deserializer import (
    VoteForArticleCommandDeserializer
)
from myapp.application.adapter.api.vote_for_article_use_case import (
...
from myapp.application.util import assert_never
    ...
    def _build_response(self, voting_result: VoteForArticleResult) -> Response:
        ...
        match voting_result:
            ...
            case AlreadyVotedResult(article_id, user_id):
                detail = f"User \"{user_id}\" has already voted for article \"{article_id}\""
                response = problem_response(
                    title="Cannot vote for an article",
                    detail=detail,
